...
class QuantumKernel(gpytorch.kernels.Kernel):
    def __init__(self, n_qubits, n_layers, phi, encoding_scale, **kwargs): #constructor
    ...
    def feature_map(self, x, params):
        ...
        for j in range(self.n_layers):
            for i in range(self.n_qubits):
                qml.RX(params[i] * self.encoding_scale * self.phi(x), wires=i)
    ...
    def forward(self, x1, x2, diag=False, **params):
        ...
        """
        kernel_matrix = torch.zeros(size=(len(x1), len(x2)))
